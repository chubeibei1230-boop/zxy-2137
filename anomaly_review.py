...
def render_anomaly_review_page(raw_df):
    ...
    base_df, detail_df = render_anomaly_filters(raw_df)

    if base_df.empty:
        st.warning("当前筛选条件下无数据，请调整筛选条件。")
        return

    anomaly_overview = compute_anomaly_overview(base_df)
    suggestions = generate_anomaly_suggestions(base_df)
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["异常概览与归类", "重点明细与复盘", "变化趋势", "运营建议"])
    ...
    with tab1:
        ...
        with col2:
            dim_df = compute_anomaly_by_dimension(base_df, dimension)
    ...
    with tab2:
        st.markdown(f"#### {view_mode}异常明细")
        tagged_detail = compute_anomaly_tags(detail_df)
        render_anomaly_detail_table(tagged_detail, dimension=dimension)

        st.divider()
        st.markdown(f"#### 单{view_mode[1:]}复盘")
        anomaly_items = tagged_detail[tagged_detail["是否异常"]][dimension].unique().tolist() if not tagged_detail.empty and tagged_detail["是否异常"].any() else []
        if not anomaly_items:
            anomaly_items = tagged_detail[dimension].unique().tolist() if not tagged_detail.empty else []
        if anomaly_items:
            selected_item = st.selectbox(
                f"选择{view_mode[1:]}查看复盘",
                options=anomaly_items,
                key="anomaly_single_item",
            )
            if selected_item:
                render_dimension_review(base_df, selected_item, dimension=dimension)
        else:
            st.info("当前筛选范围内无数据，无需复盘。")

    with tab3:
        trend_df = compute_anomaly_trends(base_df, freq="W")
    ...
    st.divider()
    render_anomaly_export_section(base_df, tagged_detail, anomaly_overview, suggestions)
